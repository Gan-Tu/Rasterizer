...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  struct line_test {
    float _x0, _y0, dx, dy;
    line_test(float x0, float y0, float x1, float y1) :
        _x0(x0), _y0(y0), dx(x1 - x0), dy(y1 - y0) {};
    int getEdgeCase(float x, float y) {
      float check = -(x-_x0)*dy + (y-_y0)*dx;
      if (check < 0) {
        return -1;
      } else if (check == 0) {
        return 0;
  ...
  line_test line1 = line_test(x0, y0, x1, y1);
  line_test line2 = line_test(x1, y1, x2, y2);
  line_test line3 = line_test(x2, y2, x0, y0);
  ...
  float sample_dist = 1.0 / sqrt(sample_rate);

  // barycentric coordinates
  struct barycoordinate {
    float xb, yb, ycb, xcb, divisor;
    barycoordinate( float x0, float y0, 
                    float x1, float y1, 
                    float x2, float y2) :  
        xb(x1), yb(y1), ycb(y2-y1), 
        xcb(x2-x1), divisor(-(x0-x1)*(y2-y1)+(y0-y1)*(x2-x1)) {}
    float get(float x, float y) {
      return (-(x-xb)*ycb + (y-yb)*xcb) / divisor;
    }
  };
  barycoordinate alpha_bary = barycoordinate(x0, y0, x1, y1, x2, y2);
  barycoordinate beta_bary = barycoordinate(x1, y1, x2, y2, x0, y0);

  Vector3D p_bary = Vector3D(), p_dx_bary = Vector3D(), p_dy_bary = Vector3D();
  SampleParams sp = SampleParams();
  sp.psm = psm;
  sp.lsm = lsm;

  // grid sampling
  int sy, sx, sub_r, sub_c, edgeCase;
  float r, c;
  // looping through each pixel
  for (sy = row_l; sy < row_h; sy++) {
    for (sx = col_l; sx < col_h; sx++) {
      //looping through each sub_pixel
      for (r = sy + sample_dist / 2, sub_r = 0; r < sy + 1; r += sample_dist, sub_r++) {
        for (c = sx + sample_dist / 2, sub_c = 0;c < sx + 1; c += sample_dist, sub_c++) {
          // check for edge
          edgeCase = line1.getEdgeCase(c, r);
          if (edgeCase == 0 || (edgeCase == line2.getEdgeCase(c, r) && edgeCase == line3.getEdgeCase(c, r))) {
            if (tri != NULL) {
              p_bary.x = alpha_bary.get(c, r);
              p_dx_bary.x = alpha_bary.get(c+1, r);
              p_dy_bary.x = alpha_bary.get(c, r+1);

              p_bary.y = beta_bary.get(c, r);
              p_dx_bary.y = beta_bary.get(c+1, r);
              p_dy_bary.y = beta_bary.get(c, r+1);

              p_bary.z = 1 - p_bary.x - p_bary.y;
              p_dx_bary.z = 1 - p_dx_bary.x - p_dx_bary.y;
              p_dy_bary.z = 1 - p_dy_bary.x - p_dy_bary.y;

              samplebuffer[sy][sx].fill_color(sub_r, sub_c, tri->color(p_bary, p_dx_bary, p_dy_bary, sp));
            } else {
              samplebuffer[sy][sx].fill_color(sub_r, sub_c, color);
            }
          }
        }
      }
